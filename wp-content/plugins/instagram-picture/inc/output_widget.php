...
?>
    ...
    <p>Style-ID:<br><input  placeholder="200 - 231" type="text" value="<?php echo $config['style'];  ?>" name="<?php echo  $this->get_field_name("style"); ?>" id="<?php  echo $this->get_field_id("style") ?>"></p>
    </label>
...
function widget($instance, $instagram)
{
   ...
   */
	...
	foreach( $wpdb->get_results("SELECT * FROM $table_name_bilder WHERE status ='0' ORDER BY id DESC LIMIT $limit") as $key => $row) 
   {
   	...
   	$ausgabe = str_replace("[$i]", $link_anfang.'<img src="'.$url.'" '.$title_ausgabe.$border_ausgabe.' alt="'.$title.'" />'.$link_ende, $ausgabe);
   	$i++;
   }
